		...
		if (cmd.getName().equalsIgnoreCase("royalauth")) {
			...
			case "help":
				...
				cs.sendMessage(ChatColor.GRAY + "  /" + label + " login [player]" + ChatColor.BLUE
						+ " - " + Language.HELP_LOGIN);
				cs.sendMessage(ChatColor.GRAY + "  /" + label + " logout [player]" + ChatColor.BLUE
						+ " - " + Language.HELP_LOGOUT);
				cs.sendMessage(ChatColor.GRAY + "  /" + label + " register [player] [password]"
						+ ChatColor.BLUE + " - " + Language.HELP_REGISTER);
				cs.sendMessage(ChatColor.GRAY + "  /" + label + " reload" + ChatColor.BLUE + " - "
						+ Language.HELP_RELOAD);
			...
			case "changepassword": {
				if (args.length < 3) {
					cs.sendMessage(ChatColor.RED + Language.NOT_ENOUGH_ARGUMENTS.toString() + " "
							+ Language.TRY + " " + ChatColor.GRAY + "/" + label + " help"
							+ ChatColor.RED + ".");
					return true;
				}
				...
				if (ap.setPassword(args[2], Config.passwordHashType))
					...
					cs.sendMessage(ChatColor.RED
							+ Language.PASSWORD_COULD_NOT_BE_CHANGED.toString());
				break;
			}
			case "login": {
				if (args.length < 2) {
			...
			case "register": {
				...
				if (ap.setPassword(rawPassword, Config.passwordHashType))
					cs.sendMessage(ChatColor.BLUE
							+ String.format(Language.REGISTERED_SUCCESSFULLY.toString(),
									ChatColor.GRAY + name + ChatColor.BLUE));
				else
					cs.sendMessage(ChatColor.RED
							+ String.format(Language.COULD_NOT_REGISTER.toString(),
									ChatColor.GRAY + name + ChatColor.RED));
				break;
			}
			case "reload":
				...
				break;
			default:
				cs.sendMessage(ChatColor.RED + Language.INVALID_SUBCOMMAND.toString() + " "
						+ Language.TRY + " " + ChatColor.GRAY + "/" + label + " help"
						+ ChatColor.RED + ".");
				break;
			}
