...
public class CmdChangePassword implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("passwd")) {
...
//			return true;
			boolean ok = false;
			String newSalt = RUtils.genSalt();
			try {
				ok = ap.checkPassword(args[0]);
				newPassword = Hasher.encrypt(newPassword, newSalt, Config.passwordHashType);
			} catch (NoSuchAlgorithmException e) {
				cs.sendMessage(ChatColor.RED + Language.ADMIN_SET_UP_INCORRECTLY.toString());
				cs.sendMessage(ChatColor.RED + Language.YOUR_PASSWORD_COULD_NOT_BE_CHANGED.toString());
				return true;
			...
			ap.setHashedPassword(newPassword, newSalt, Config.passwordHashType);
			cs.sendMessage(ChatColor.BLUE + Language.YOUR_PASSWORD_CHANGED.toString());
			return true;
		}
		return false;
